...
const CodeChefContestGrid: React.FC<CodeChefContestGridProps> = ({ 
  ...
  const stats = useMemo(() => {
    ...
    const attendanceRate = contestHistory.length > 0 ? (attended / contestHistory.length) * 100 : 0;
  ...
  const getStatusIcon = (contest: ContestEntry) => {
    if (contest.attended === false) {
      return <XCircle className="w-4 h-4 text-red-500" />;
    }
    if (contest.problemsCount && contest.problemsCount > 0) {
      return <CheckCircle className="w-4 h-4 text-green-500" />;
    }
    return <Target className="w-4 h-4 text-[#2DD4BF]" />;
  };
    ...
    return (
      <Card className={`bg-card border-border/50 ${className}`}>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">CodeChef contest history not found.</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-bold text-foreground">Contest History</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-[#E6FFFA] dark:bg-cyan-500/20 rounded-lg p-4 border border-[#A7F3D0] mt-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-[#0F766E]">{Math.round(stats.attendanceRate)}%</div>
