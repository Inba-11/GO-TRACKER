...
interface CodeChefContestHistoryProps {
...
const CodeChefContestHistory: React.FC<CodeChefContestHistoryProps> = ({
  contestHistory = [],
  className = ""
}) => {
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-600 flex-wrap">
                    ...
                    {contest.rank && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium text-indigo-600">#{contest.rank.toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {contest.problemsSolved && contest.problemsSolved.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {contest.problemsSolved.slice(0, 5).map((problem, idx) => (
                        <Badge key={idx} variant="secondary" className="text-xs bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors">
                          {problem}
                        </Badge>
                      ))}
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {contest.problemsCount !== undefined && (
                    <div className="text-right">
                      <Badge variant="default" className="text-base font-bold bg-indigo-600">
                        {contest.problemsCount}
                      </Badge>
                      <div className="text-xs text-slate-600 mt-1">Problems</div>
                    </div>
                  )}
                </div>
              </div>
...
export default CodeChefContestHistory;
